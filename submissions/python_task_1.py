...
def verify_time_completeness(data):
    try:
        data['start_time'] = pd.to_datetime(data['startDay'] + ' ' + data['startTime'], format='%A %H:%M:%S')
        data['end_time'] = pd.to_datetime(data['endDay'] + ' ' + data['endTime'], format='%A %H:%M:%S')
    except Exception as e:
        print(f"Error: {e}. Please check the format of the date and time strings in your dataset.")
        return None

    data['duration'] = data['end_time'] - data['start_time']
    completeness_check = data.groupby(['id', 'id_2']).apply(lambda x: x['duration'].sum() >= pd.Timedelta(days=7) and 
                                                             x['start_time'].dt.hour.min() == 0 and
                                                             x['start_time'].dt.hour.max() == 23 and
                                                             set(x['start_time'].dt.dayofweek) == set(range(7)))
    return completeness_check


file_path = 'C:\\Users\\ayush\\Downloads\\dataset-2.csv'
data = pd.read_csv(file_path)
